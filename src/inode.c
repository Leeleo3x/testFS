...
#define inode_hashfn(nr) hash_int((unsigned int)nr, INODE_HASH_SHIFT)
...
static struct inode *inode_hash_find(struct super_block *sb, int inode_nr) {
  struct hlist_node *elem;
  struct inode *in;

  hlist_for_each_entry(in, elem, &inode_hash_table[inode_hashfn(inode_nr)],
                       hnode) {
...
static void inode_hash_remove(struct inode *in) { hlist_del(&in->hnode); }
...
/*
 ...
 if did not find, allocate new inode in memory, make count 1, assign inode
 number read the inode from the
 */
...
inline int testfs_inode_get_size(struct inode *in) { return in->in.i_size; }

inline inode_type testfs_inode_get_type(struct inode *in) {
  return in->in.i_type;
}

inline int testfs_inode_get_nr(struct inode *in) { return in->i_nr; }
...
int testfs_write_data_no_lock(struct inode *in, int start, char *buf, const int size) {
  testfs_write_data(in, start, buf, size);
}
#else
int testfs_write_data_no_lock(struct inode *in, int start, char *buf, const int size) {
...
int testfs_write_data(struct inode *in, int start, char *buf, const int size) {
  testfs_write_data_no_lock(in, start, buf, size);
